...
export class AddressService implements AddressServiceInterface {
  private getAddressByStringPreUrl = 'http://localhost:8181/search.php?q=';
  private getAddressByStringPostUrl =
    '&countrycodes=fr&limit=5&format=jsonv2&addressdetails=1&layer=address&viewbox=49.0487,2.7521,48.7598,2.1121';
  private getAddressByCoordsPreUrl = 'http://localhost:8181/reverse?';
  private getAddressByCoordsPostUrl =
    '&zoom=18&format=jsonv2&addressdetails=1&accept-language=fr&layer=address';
  ...
  matchingSchoolDeparture(lat: number, lon: number): string | undefined {
    const school = this.$schoolList.find(
      (school) => school.lat === lat && school.lon === lon,
    );
  ...
  getFormattedAddress(param: string | number[]): Observable<string> {
    if (typeof param === 'string') {
      return this.getAddressByString(param).pipe(
        map((addressObject) => {
          const schoolExist = this.matchingSchoolDeparture(
            addressObject.lat,
            addressObject.lon,
          );
          if (schoolExist) {
      ...
      const schoolExist = this.matchingSchoolDeparture(param[0], param[1]);
      if (schoolExist) {
