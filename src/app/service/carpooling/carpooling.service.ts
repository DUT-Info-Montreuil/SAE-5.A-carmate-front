...
export class CarpoolingService
  ...
  implements CarpoolingServiceInterface
{
  ...
  search(search: Search): void {
    this.http
      .get<{
        carpoolings_route: Carpooling[];
        nb_carpoolings_route: number;
  ...
  createSubscription(subscription: CreateSubscriptionPayload): Observable<any> {
    return this.http.post<any>(
      `${environment.path}/regular/${subscription.max_passengers ? 'carpooling' : 'booking'}`,
      subscription,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('auth_token')}`,
  ...
  getSubscriptions(): Observable<any> {
    return this.http.get<Subscription[]>(
      `${environment.path}/history/schedule-carpooling/booked`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('auth_token')}`,
        },
  ...
  getPublishedSubscriptions(): Observable<any> {
    return this.http.get<PublishedSubscription[]>(
      `${environment.path}/history/scheduled`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('auth_token')}`,
        },
  ...
  getPublishedCarpoolings(): Observable<any> {
    return this.http.get<publishedCarpooling[]>(
      `${environment.path}/history/carpooling/published`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('auth_token')}`,
        },
  ...
  postCode(passengerCode: number, carpoolingId: number): Observable<any> {
    return this.http.post<publishedCarpooling[]>(
      `${environment.path}/carpooling/confirm`,
      {
  ...
  book(id: number, is_scheduled?: boolean, date?: number): Observable<void> {
    return this.http.post<void>(
      `${environment.path}/carpooling/book`,
      {
        carpooling_id: id,
