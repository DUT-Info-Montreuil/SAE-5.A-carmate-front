...
export class PublishedCarpoolingsComponent {
  ...
  ngOnInit(): void {
    ...
    this.addressService
      .getFormattedAddress(this.publishedCarpooling.starting_point)
      .subscribe((address) => {
        this.carpoolingToDisplay.departure = address;
      });
    this.addressService
      .getFormattedAddress(this.publishedCarpooling.destination)
      .subscribe((address) => {
    ...
    this.carpoolingToDisplay.isOutdated = moment(date).isBefore(
      moment(),
      'day',
    );
  }
