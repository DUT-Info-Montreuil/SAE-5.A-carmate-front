...
export class PublishedSubscriptionsComponent {
  ...
  ngOnInit(): void {
    this.addressService
      .getFormattedAddress(this.publishedSubscription.starting_point)
      .pipe(take(1))
      .subscribe((startingPoint) => {
        this.startingPointToDisplay = startingPoint;
      });
    this.addressService
      .getFormattedAddress(this.publishedSubscription.destination)
      .pipe(take(1))
  ...
  private generatePendingCarpoolings(): void {
    ...
    while (currentDate.unix() <= this.publishedSubscription.end_date / 1000) {
      if (
        this.publishedSubscription.days.includes(
          currentDate.format('dddd') as WeekDay,
        )
      ) {
    ...
    const carpoolingDates = this.publishedSubscription.carpools.map((c) =>
  ...
  private generateBookedCarpoolings(): void {
    this.publishedSubscription.carpools
      ...
      .forEach((carpool) => {
        ...
        bookedCarpoolingToDisplay.passengers = carpool.passengers_profile.map(
          (passenger) => {
