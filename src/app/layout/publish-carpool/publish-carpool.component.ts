...
export class PublishCarpoolComponent {
  ...
  submit(): void {
    const dateStart: Date = new Date(
      this.publishForm.get('departure_date')!.value!,
    );
    const timeString: string = this.publishForm.get('departure_time')!.value!;
    const timeParts = timeString.split(':');

    dateStart.setHours(parseInt(timeParts[0], 10), parseInt(timeParts[1], 10));

    const departureDateTimeUnix = dateStart.getTime() / 1000;
