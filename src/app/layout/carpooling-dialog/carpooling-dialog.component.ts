...
export class CarpoolingDialogComponent {
  ...
  ngOnInit() {
    this.scoreboardService
      .getScoreUserData(this.data.carpooling.driver_id)
      .subscribe({
        next: (score) => {
  ...
  submit(): void {
    this.carpoolingService
      .book(
        this.data.carpooling.id,
        this.data.carpooling.is_scheduled,
        parseInt(this.data._searchParams.departure_date_time),
      )
      .subscribe({
        next: () => {
          this.dialogRef.close();
        ...
        error: (error: HttpErrorResponse) => {
          switch (error.status) {
            ...
            case 403:
              this.notifier.error(
                "Vous 'n'êtes plus autorisé à éffectuer cette action",
              );
              break;
            case 404:
            ...
            case 423:
              this.notifier.error(
                'La prise en charge de ce covoiturage est passée.',
              );
              break;
            case 500:
              this.notifier.error('Erreur interne.');
