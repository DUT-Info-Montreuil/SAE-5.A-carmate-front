...
export class CarpoolingComponent {
  ...
  ngOnInit() {
    const dateTime = new Date(this._carpooling.departure_date_time);
    dateTime.setHours(dateTime.getHours() - 1);
    this.departure_date = dateTime.toLocaleDateString('fr-FR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    });
    this.departure_time = dateTime.toLocaleTimeString('fr-FR', {
      hour: '2-digit',
      minute: '2-digit',
    });

    forkJoin({
      starting_point: this.addressService.getFormattedAddress(
        this._carpooling.starting_point,
      ),
      destination: this.addressService.getFormattedAddress(
        this._carpooling.destination,
      ),
