...
export class CreateSubscriptionComponent {
  ...
  subForm = new FormGroup({
    ...
    max_passengers: new FormControl(1, [
      Validators.required,
      Validators.min(1),
      Validators.max(4),
    ]),
  ...
  constructor(
    ...
    protected authenticationService: AuthenticationServiceInterface,
  ) {}
        ...
        .subscribe((addresses) => {
          if (Array.isArray(addresses) && addresses.length > 0) {
            ...
            for (let index = 0; index < addresses.length; index++) {
              displayResults.push(
                this.addressService.formatAddress(addresses[index]),
              );
            }
  ...
  submit() {
    const date_start: Date = new Date(this.subForm.get('date_start')!.value!);
    const date_end: Date = new Date(this.subForm.get('date_end')!.value!);
    ...
    const payload: CreateSubscriptionPayload = {
      ...
      days: this.subForm
        .get('days')!
        .getRawValue()
        .map((day: WeekDay) => this.daysMapping[day]),
    ...
    this.carpoolingService.createSubscription(payload).subscribe({
      next: () => {
        ...
        this._snackBar
          .open(
            'Souhaitez-vous regarder le résultat de votre abonnement ?',
            'Voir',
            {
              duration: 5000,
            },
